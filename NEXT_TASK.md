...
Current Status:
Task 18AA fixed the Chapter `366` quote-boundary text blocker and generated Speaker Assignment Draft `13` for operator review. No Casting Plan, job, TTS, artifact, or audio was created.
...
- Chapter `366` speaker draft state: exactly one current non-stale draft, Draft `13`, `status = generated`, `target_count = 10`, `valid_count = 10`, `invalid_count = 0`, `remaining_unreviewed_count = 10`, `cache_hit_count = 0`, `cache_miss_count = 1`
- Chapter `366` downstream state: Casting Plans `0`, jobs `0`, job_chapters `0`, segments `0`, segment attempts `0`, repair blocks `0`, artifacts `0`, active audio `none`
...
Task 18AB - Review Chapter 366 Speaker Assignments and Create an Unapproved Final Voice Map.

Why:
- Chapter `366` is now past the text blocker and has one valid current speaker draft ready for human review.
- The next safe boundary is reviewing Draft `13` and creating one unapproved Final Voice Map only after every speaker row is addressed.
- Creating another Chapter `366` speaker draft, starting a job, or generating audio would be a duplicate or premature mutation.

Scope:
1. Re-verify canonical runtime and Chapter `366` Draft `13` state before mutation.
2. Review all `10` generated speaker rows, paying special attention to merged quote `u0004-c739867fa093`.
3. Create exactly one unapproved Final Voice Map / Casting Plan draft from reviewed Draft `13`.
4. Stop before Casting Plan approval, job preparation, TTS preview, TTS synthesis, or audio rendering.
...
- Do not approve a Casting Plan, prepare a job, or start any job during Task `18AB`.
