...
ADR-013 chốt kiến trúc voice mục tiêu là Book Voice Profile ba nhóm (narrator/male/female) cùng unknown fallback và optional character override. Schema v2 hiện vẫn dùng character voice bắt buộc; Three-Voice Profile chưa triển khai.
...
- Chọn preset voice VieNeu; manual/multi-voice casting hiện có và Three-Voice Profile cấp book là bước tiếp theo.
...
Schema thực thi hiện tại là version 2; xem `docs/DATA_MODEL.md` và `story_audio/migrations/`. Các entity chính:
...
- `characters`: hiện có `display_name` và `default_voice_id NOT NULL`; đây là current implementation, không phải model Three-Voice cuối cùng.
...
Model dự kiến của task tiếp theo:
...
Chưa có BookVoiceProfile/gender/optional override trong schema v2. Giá trị `characters.default_voice_id` cũ phải được bảo toàn như legacy override. Job cũ không được resolve lại.
...
- Bước kế tiếp là Book Voice Profile gồm narrator/male/female/unknown và optional character override.
...
- Mỗi thay đổi schema DB đều có forward-only SQL migration checksum-locked; migration kế tiếp nếu cần là `0003_*`.
...
- Book Voice Profile ba nhóm/unknown resolver (task tiếp theo, chưa phải feature hiện có).
