...
- Chọn giọng dựng sẵn hoặc giọng clone của VieNeu-TTS.
...
|---|---|---|
| UI | React + TypeScript + Vite | UI trạng thái phức tạp, dễ mở rộng và kiểm thử |
| Backend | FastAPI + Pydantic | Phù hợp hệ sinh thái Python của VieNeu |
| Cơ sở dữ liệu | SQLite, WAL mode, SQLAlchemy + Alembic | Không cần cài server DB, có migration và transaction |
...
| Kiểm thử | pytest + Vitest + Playwright | Kiểm thử từng lớp và luồng UI chính |
...
```text
backend/app/
├── api/                 # HTTP endpoints, SSE; không chứa nghiệp vụ
├── application/         # Use cases: import book, queue chapters, resume job
├── domain/              # Entity, enum trạng thái, quy tắc chuyển trạng thái
├── infrastructure/
│   ├── db/              # SQLAlchemy repositories, migrations
│   ├── epub/            # Bộ đọc EPUB
│   ├── tts/             # VieNeu adapter
│   ├── audio/           # Ghép, encode, kiểm tra audio
│   └── storage/         # Đường dẫn và thao tác file an toàn
├── worker/              # Durable queue consumer và job runner
├── config.py            # Cấu hình từ .env/config.yaml
└── main.py
```
...
- `id`, `title`, `author`, `source_path`
- `source_sha256`, `cover_path`, `chapter_count`
- `import_status`, `created_at`, `updated_at`

### `chapters`

- `id`, `book_id`, `spine_index`, `title`
- `source_href`, `plain_text`, `text_sha256`, `char_count`
- `audio_status`, `audio_path`, `duration_ms`
- Unique: `(book_id, spine_index)`

### `voices`

- `id`, `kind` (`preset` hoặc `clone`), `display_name`
- `engine`, `engine_voice_id`
- `reference_audio_path`, `reference_sha256`
- `settings_json`, `created_at`

### `jobs`

- `id`, `book_id`, `status`, `priority`
- `voice_id`, `settings_snapshot_json`
- `total_chapters`, `completed_chapters`, `total_segments`, `completed_segments`
...
- `id`, `job_id`, `chapter_id`, `sequence`, `status`
- `text_sha256`, `output_path`, `duration_ms`, `error_message`
- Unique: `(job_id, chapter_id)`

### `segments`

- `id`, `job_chapter_id`, `segment_index`, `text`
- `text_sha256`, `status`, `attempt_count`
- `wav_path`, `duration_ms`, `audio_sha256`, `error_message`
- Unique: `(job_chapter_id, segment_index)`

`settings_snapshot_json` phải lưu toàn bộ model, voice, temperature, top-k, max-chars, định dạng và phiên bản chunker. Công việc cũ vì vậy không đổi hành vi khi người dùng sửa thiết lập mặc định.
...
Tên thật hiển thị trong metadata; tên file được làm sạch để tránh ký tự không hợp lệ trên Windows. Đường dẫn lưu tương đối trong DB để có thể di chuyển cả thư mục dự án.
...
- Thêm giọng clone bằng file WAV 3–5 giây, đặt tên và nghe thử.
- Không cho sửa/xóa giọng đang được job dùng; cho phép nhân bản cấu hình.
- Cảnh báo chỉ dùng giọng clone khi có quyền của chủ giọng.
...
- Mỗi thay đổi schema DB đều có Alembic migration.
...
### Giai đoạn 1 — MVP bền vững

- Import EPUB và hiển thị 1.980 chương.
- Chọn preset voice, preview và tạo một/nhiều chương.
- SQLite queue, một worker, checkpoint cấp segment.
- Ghép một file M4A/MP3 cho mỗi chương.
- UI tiến độ, pause/resume/retry và player.

### Giai đoạn 2 — Hoàn thiện sử dụng lâu dài

- Voice clone manager, chia chương theo thời lượng.
- Dự báo thời gian/dung lượng, dọn file tạm tự động.
- Tìm kiếm chương, hàng đợi ưu tiên, xuất manifest.
- Backup/restore DB và cấu hình.

### Giai đoạn 3 — Mở rộng

- Worker chạy trên máy khác qua adapter hàng đợi.
- Thêm engine TTS khác.
- Đồng bộ phụ đề theo segment/câu.
- Xuất audiobook M4B, RSS/podcast hoặc pipeline YouTube.
...
- Trước job, ước tính dung lượng audio tạm, ảnh và video; cảnh báo nếu không đủ headroom.
- Mặc định yêu cầu còn tối thiểu 15 GB hoặc ngưỡng cấu hình.
- Worker kiểm tra free space trước mỗi chapter và trước compose video.
...
- Không tự xóa master audio, subtitle, manifest hoặc final video đang active.
...
### 15.11 Scene, image và video edge cases

- Scene planner thiếu hoặc lặp segment: validator từ chối timeline.
- Scene quá ngắn/dài: normalize tại ranh giới segment; không tự bịa timestamp.
- Gemini prompt ảnh thay đổi nhân vật: character/location bible được chèn theo ID ổn định; prompt lưu snapshot.
- Image provider timeout/quota/login: retry riêng scene, giữ các ảnh thành công.
- Ảnh trùng: perceptual hash cảnh báo và đưa scene về retry/review.
- Ảnh sai tỷ lệ/kích thước/hỏng: normalize hoặc reject trước compose.
- Thiếu ảnh một scene: cho chọn retry, dùng placeholder có cảnh báo, hoặc dừng compose; mặc định là dừng.
- Video compose lỗi: giữ audio, timeline, subtitle và ảnh; chỉ retry compose.
- Video duration lệch audio: verification failed; không đánh dấu final.
- Người dùng đổi một ảnh: chỉ invalidates compose/video, không tạo lại audio hoặc scene khác.

### 15.12 Nhiều job và tranh chấp tài nguyên

- Chỉ một VieNeu CPU worker trên máy hiện tại.
- Gemini/image network workers có giới hạn concurrency và rate limit riêng.
- FFmpeg compose không chạy đồng thời với TTS nếu benchmark cho thấy tranh CPU/RAM; scheduler dùng resource slots.
...
| Retry image | Chỉ tạo lại scene được chọn |
...
| Change image | Chỉ compose lại video |
...
- Sửa text, visual profile, ảnh và output format; kiểm tra invalidation đúng phạm vi.
- Khởi động lại sau khi DB có job `running` nhưng heartbeat đã hết hạn.

## 16. Phạm vi MVP đã chốt

MVP kết thúc ở audio theo chương có checkpoint và **bao gồm Gemini punctuation repair**. Image generation và video composition vẫn để ngoài MVP.
...
- Segment timeline manifest để dùng cho subtitle/image/video sau MVP.
...
- Scene planning và prompt ảnh.
- Image generation.
- Video composition và YouTube metadata/thumbnail.
