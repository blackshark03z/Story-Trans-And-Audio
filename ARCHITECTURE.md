...
Current roadmap state: Story Audio is production-ready and operates under the Daily Production UX roadmap. `DAILY-PROD-1` through `DAILY-PROD-4` are complete, `DAILY-PROD-5A` is complete, `DAILY-PROD-5B Phase 1` is complete, `DAILY-PROD-5B Phase 2` is complete, `DAILY-PROD-5B Phase 3` is complete, and `DAILY-PROD-5 - Batch Approval, Prepare, Render And QA Closeout` remains the active system milestone. The exact authorized task is defined in `NEXT_TASK.md`; the next bounded step is isolated schema-13 activation and durable request-store integration validation on temporary databases only. Schema 13 exists only as a dormant migration, canonical/default schema remains 12, PREPARE execution remains unauthorized, START_RENDER remains separate, and Chapter 369 remains paused production/editorial work, not a roadmap driver.
...
Schema thực thi hiện tại phải được xác minh từ `/api/runtime` và các migration trong `story_audio/migrations/`; tại lần reconciliation 2026-07-20 runtime canonical báo schema `12`. Các entity chính:
...
- `speaker_assignment_drafts` + content-addressed JSON: Gemini suggestions pin TextRevision/Character Bible/context và không tự sửa Casting Plan.
...
- Mỗi thay đổi schema DB đều có forward-only SQL migration checksum-locked. Migration kế tiếp phải lấy số tiếp theo sau migration cao nhất hiện có trong `story_audio/migrations/`; không suy diễn từ tài liệu lịch sử.
