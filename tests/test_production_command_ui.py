...
class ProductionCommandUiTests(unittest.TestCase):
    ...
    def test_milestone_one_uses_verified_runtime_readiness_and_one_journey_primary(self) -> None:
        ...
        self.assertIn("if(!item.character_id){await returnToVoiceConfiguration()", self.js)
        self.assertNotIn("if(!item.character_id){setAppRoute('voices')", self.js)
        self.assertIn('value="${esc(value)}">${esc(label)}</option>', self.js)
        self.assertNotIn('value="${esc(value)}\n    "', self.js)
