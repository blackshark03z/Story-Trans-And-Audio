...
class ApprovedReviewFixtureHandler(SpeakerReviewWorkspaceFixtureHandler):
    @classmethod
    def queue(cls) -> dict:
        unresolved_key = "unresolved-dialogue:1001:fixture-approved"
        source = {
            ...
            "suggested_voice_id": "commander",
        }
        review = {
            "decision": "ACCEPTED",
            "reviewer_payload": {"proposed_resolution": "EXISTING_CHARACTER", "existing_character_id": 25},
            "source_suggestion": source,
            "resulting_mapping": {"character_id": 25, "speaker_key": unresolved_key},
        }
        return {
            "schema": "story-audio-gemini-speaker-review-queue/v1",
            "status": "ready_for_human_review",
            "analysis_run_id": "fixture-approved-run",
            "suggestions": [
                {
                    **source,
                    "source_analysis_run_id": "fixture-approved-run",
                    "review_state": "ACCEPTED",
                    "human_review": review,
                }
            ],
            "summary": {"total": 1, "pending_review": 0, "approved": 1},
        }
        ...
        def row(*, key: str, role: str, name: str, character_id: int | None) -> dict:
            ...
            return {
                ...
                "actions": {
                    ...
                    "can_create_range_or_chapter_override": character_id is not None,
...
'''
        ...
        self.assertIn("Duyệt người nói hàng loạt", evidence["completedBatch"])
        self.assertIn("0 chờ duyệt", evidence["completedBatch"])
        self.assertIn("Đã hoàn tất", evidence["completedBatch"])
        self.assertEqual(evidence["completedBatchActions"], 0)
        self.assertTrue(evidence["voiceBatchInStepTwo"])
        self.assertIn("Duyệt giọng Gemini hàng loạt", evidence["voiceBatch"])
        self.assertIn("Commander Voice", evidence["voiceBatch"])
        self.assertIn("1 chờ duyệt", evidence["voiceBatch"])
        self.assertEqual(evidence["voiceBatchButtons"], 3)
        ...
        self.assertFalse(evidence["perRoleSave"])
        self.assertIn("Không có thay đổi chưa lưu", evidence["batchStatus"])
