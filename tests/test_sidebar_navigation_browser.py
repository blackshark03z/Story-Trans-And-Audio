...
const {boundedBrowserTimeout}=require('./scripts/browser_acceptance_runtime.cjs'),delay=ms=>new Promise(resolve=>setTimeout(resolve,ms)),poll=async fn=>{const end=Date.now()+boundedBrowserTimeout(12000);let error;while(Date.now()<end){try{const value=await fn();if(value)return value}catch(e){error=e}await delay(50)}throw error||Error('Timed out')};
