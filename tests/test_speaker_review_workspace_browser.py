...
class SpeakerReviewWorkspaceBrowserTests(unittest.TestCase):
    ...
    def test_review_workspace_real_browser_certification(self) -> None:
        ...
        self.assertTrue(evidence["redundantShortcutsRemoved"])
        self.assertTrue(evidence["directDecisionControlsAvailable"])
        self.assertTrue(evidence["voiceEditingDeferred"], evidence["voiceEditingEvidence"])
        ...
        self.assertEqual(evidence["correctionEvidence"]["voiceControls"], 0)
        self.assertTrue(evidence["correctionEvidence"]["reviewOpen"])
        self.assertFalse(evidence["correctionEvidence"]["voicesOpen"])
        self.assertIn("Giọng hiện có được giữ nguyên", evidence["correctionEvidence"]["result"])
