...
- Repaired legacy UTF-8/Windows-1252 mojibake in user-facing JavaScript,
  API/pipeline messages, and project documentation without changing canonical
  story text or runtime data.
- Replaced the eight equally prominent Production cards with four operator
  phases while retaining the existing backend resolver states and safety
  boundaries.
- Moved casting review, preparation, render monitoring/recovery, and QA into
  the main Production workspace. Detailed assignments and technical metadata
  are collapsed; the long operational modal is no longer used.
- Added concise casting summaries, separate `Chuẩn bị` and
  `Bắt đầu render` states, segment progress for active/recoverable jobs, and
  clear `Chấp nhận` / `Cần sửa` QA actions.
- Added UTF-8 source regression checks and a dependency-free real Chromium
  smoke for casting review, approved, prepared, running, recoverable-failure,
  and QA states at both required viewport sizes.
- Canonical Chapter `369` was inspected read-only and remains unchanged. No
  PREPARE, START_RENDER, QA, provider/TTS, Job, Artifact, or audio mutation was
  performed.
...
- **DAILY-PROD-2B2-D1 - Contextual voice detour activation stability**: fixed a real-browser hang when activating the Voice Library detour from Production.
  ...
  - **Historical next task at that checkpoint**: `DAILY-PROD-3A` - Audio Library Completed Output List And Playback Entry.
...
- **DAILY-PROD-5B Phase 8 - Same-Transaction PREPARE Adapter Integration Design**: completed the design/model checkpoint and committed it as `24087732b8a05d94eaf5a3af2c743602123923e8`.
  ...
  - **Recovery**: kept duplicate invocation, ambiguous commit, legacy unlinked Job, no-second-Job, worker exclusion, and START_RENDER separation fail-closed and design-only.
  - **Review and validation**: six parallel read-only reviews were reconciled; focused model tests passed (`90`), affected suite passed (`198`), full offline suite passed (`1447` tests, `1` skipped), Doctor passed with `critical_errors=0`, and runtime/UI syntax checks passed.
  - **Canonical safety**: schema/latest remained `12 / 12`; dormant request/linkage tables remained absent; canonical DB SHA-256 `dba41f6eb3eaba5de4a4d9964f41ee93bb730ac8c2d6fd47df202479ad203b23`, size `4009984`, and mtime unchanged; Chapter `369` remained at zero jobs/artifacts.
  - **Decision**: Phase 9 is authorized only for isolated prerequisite resolution. Runtime adapter/orchestrator wiring, canonical migration, PREPARE API/UI execution, production Job creation, provider/Gemini/TTS, worker wake, and START_RENDER remain unauthorized.
- **DAILY-PROD-5B Phase 7 - Dormant Request-to-Job Linkage Persistence**: closed the dormant linkage implementation checkpoint with schema artifact `0014_batch_prepare_job_links.sql`, pure isolated repository behavior, request/Job uniqueness, deterministic replay/conflict lookup, transaction evidence validation, concurrency/rollback coverage, and canonical schema 12 safety.
...
- Resume test giữ 9 segment hợp lệ và tạo lại một segment lỗi.`r`n
