...
try{
  ...
  const selected=await waitFor(`(()=>{const bar=document.querySelector('[data-speaker-review-batch-bar]'),button=bar?.querySelector('[data-batch-selected-speaker-suggestions]');return bar&&button&&!button.disabled&&button.textContent.includes('(1)')?{count:bar.querySelector('[data-speaker-selected-count]')?.textContent,rules:bar.innerText.includes('Khi chấp nhận:')&&bar.innerText.includes('Khi chưa duyệt / Để sau / Chưa chắc:'),safe:bar.innerText.includes('Chấp nhận tất cả an toàn')}:null})()`);
  ...
  if(!(selected.count==='1'&&selected.rules&&selected.safe&&payloadOk&&noRender&&afterPending===beforePending-1))throw new Error(JSON.stringify({selected,beforePending,afterPending,payloadOk,noRender}));
