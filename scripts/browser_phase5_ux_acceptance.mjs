...
try {
  ...
  const show = async (taskType, width = 1366, height = 768) => {
    ...
    return evaluate(`(()=>{
      ...
      return{taskType,width:viewportWidth,height:viewportHeight,stage:document.querySelector('#productionStateBadge')?.textContent.trim(),heading:document.querySelector('#productionCurrentStepHeading')?.textContent.trim(),text,playerVisible:visible(player),qaActionsVisible:visible(qaActions),qaActionsAfterContext:!!(playerRect&&decisionRect&&qaRect&&qaRect.top>=playerRect.bottom&&qaRect.top>=decisionRect.bottom),qaLabels:[...document.querySelectorAll('#productionQaActions button')].filter(visible).map(item=>item.textContent.trim()),technicalOpen:!!technical?.open,repairDetailsOpen:!!repairDetails?.open,problemBeforePlan:text.indexOf('Ghi chú QA')>=0&&text.indexOf('Ghi chú QA')<text.indexOf('Kiểm tra toàn bộ bản sửa'),repairAction:document.querySelector('#repairConfirmUnified')?.textContent.trim()||null,retentionVisible:text.includes('audio cũ sẽ bị xóa vĩnh viễn'),rawTechnicalVisible:/Revision 401|Artifact #?9901|evidence_id:77/.test(text),completePrimaryVisible:visible(primary),completePrimaryLabel:primary?.textContent.trim()||null,nextCycleVisible:visible(nextCycle),nextCycleLabel:nextCycle?.textContent.trim()||null,downloadVisible:visible(download),downloadHref:download?.getAttribute('href')||null,legacyVisible:['#flowView','#productionLegacyJobPanel','#preRenderConfigurationPanel'].some(selector=>visible(document.querySelector(selector))),horizontal:document.documentElement.scrollWidth>innerWidth+1,mutations:window.__phase5MutationCalls.slice()};
    })()`);
  };
  ...
  if (humanQa.stage !== "Đã bàn giao" || humanQa.playerVisible || humanQa.qaActionsVisible || humanQa.text.includes("Ghi chú QA") || humanQa.completePrimaryLabel !== "Mở Duyệt audio" || !humanQa.nextCycleVisible || humanQa.nextCycleLabel !== "Bắt đầu lượt sản xuất mới" || !humanQa.text.includes("Bạn có thể duyệt sau") || !humanQa.text.includes("không duyệt, xóa hay thay đổi audio và Job hiện có") || !humanQa.text.includes("Audio đã được bàn giao sang Duyệt audio") || humanQa.technicalOpen || humanQa.legacyVisible) throw new Error(`HUMAN_QA handoff failed: ${JSON.stringify(humanQa)}`);
