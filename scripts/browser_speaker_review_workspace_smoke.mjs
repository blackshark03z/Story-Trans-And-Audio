...
try {
  ...
  const evaluate = async (expression, timeoutMs = 25000) => {
    const response = await Promise.race([
      ...
      delay(timeoutMs).then(() => { throw new Error(`Runtime.evaluate timed out after ${timeoutMs}ms: ${String(expression).slice(0,220)}`); }),
    ]);
  ...
  await click('[data-generate-speaker-suggestions]');
  const analysisElapsed = await waitFor(`Number(window.storyAudioAppState.bookVoiceRegistry.speakerSuggestions.analysisProgress?.elapsedSeconds||0)>=3 && document.querySelector('[data-speaker-analysis-progress]')?.innerText.includes('giây')`, 7000);
  ...
  await delay(300);
  const jobsPollingControlStates = await evaluate(`(async()=>{const inspect=async(selector,expectFocusable=true)=>{const control=document.querySelector(selector);if(!control)throw new Error("Missing review control: "+selector);let blurCount=0;control.addEventListener("blur",()=>{blurCount+=1});if(expectFocusable)control.focus();const selection=control.matches("input,textarea")?[control.selectionStart,control.selectionEnd]:null;await loadJobs();const current=document.querySelector(selector);return{sameNode:current===control,focusExpected:expectFocusable,disabled:!!current?.disabled,focused:document.activeElement===control,blurCount,value:current?.value,selection:current?.matches("input,textarea")?[current.selectionStart,current.selectionEnd]:null,selectionBefore:selection}};const states={};states.resolution=await inspect(${JSON.stringify(attr("data-speaker-suggestion-resolution", key2))});states.group=await inspect(${JSON.stringify(attr("data-speaker-suggestion-group", key5))});states.character=await inspect(${JSON.stringify(attr("data-speaker-suggestion-character", key1))});const name=document.querySelector(${JSON.stringify(attr("data-speaker-suggestion-name", key2))});name.setSelectionRange(1,Math.min(4,name.value.length));states.name=await inspect(${JSON.stringify(attr("data-speaker-suggestion-name", key2))});states.aliases=await inspect(${JSON.stringify(attr("data-speaker-suggestion-aliases", key2))});states.filter=await inspect('[data-speaker-review-filter="confidence"]');const details=document.querySelector(${JSON.stringify(attr("data-speaker-suggestion-context", key2))});details.open=true;const detailNode=details;await loadJobs();states.details={sameNode:document.querySelector(${JSON.stringify(attr("data-speaker-suggestion-context", key2))})===detailNode,open:detailNode.open};detailNode.open=false;return states})()`);
  ...
  const backgroundGroupVisible = await evaluate(`(() => { const card=document.querySelector(${JSON.stringify(attr("data-speaker-suggestion-card", key5))}); const group=card?.querySelector(${JSON.stringify(attr("data-speaker-suggestion-group", key5))}); return !!card && card.innerText.includes("Quần chúng nam") && group?.value==="MALE" })()`);
  ...
  const correctionEvidence = await evaluate(`(() => { const card=document.querySelector(${JSON.stringify(attr("data-speaker-suggestion-card", key1))}),review=document.querySelector('[data-assignment-section="review"]'),voices=document.querySelector('[data-assignment-section="voices"]'),item=window.storyAudioAppState.bookVoiceRegistry.speakerSuggestions?.result?.suggestions?.find(row=>row.unresolved_key===${JSON.stringify(key1)}); return {history:!!item?.review_history?.some(entry=>entry.audit_event_id),voiceControls:card?.querySelectorAll('[data-speaker-suggestion-voice-mode],[data-speaker-suggestion-voice-scope],[data-speaker-suggestion-voice]').length||0,reviewOpen:!!review?.open,voicesOpen:!!voices?.open,result:window.storyAudioAppState.bookVoiceRegistry.speakerSuggestions?.lastResult||''} })()`);
  const correctionHistoryVisible = correctionEvidence.history;

  await click('[data-speaker-review-view="NEEDS_REVIEW"]');
  await click(attr("data-speaker-suggestion-select", key5));
  const batchBarSelection = await waitFor(`(() => { const bar=document.querySelector('[data-speaker-review-batch-bar]'),list=document.querySelector('.speaker-suggestion-list'),accept=bar?.querySelector('[data-batch-selected-speaker-suggestions]'); return bar&&list&&accept&&!accept.disabled&&accept.textContent.includes('(1)')?{selected:bar.querySelector('[data-speaker-selected-count]')?.textContent,hasRules:bar.textContent.includes('Nếu chấp nhận tất cả đủ điều kiện')&&bar.textContent.includes('Nếu chấp nhận mục đã chọn')&&bar.textContent.includes('Sau khi batch hoàn tất'),safeAction:bar.innerText.includes('Chấp nhận tất cả đủ điều kiện'),staticPosition:getComputedStyle(bar).position==='static',afterList:!!(list.compareDocumentPosition(bar)&Node.DOCUMENT_POSITION_FOLLOWING),noOpenDialog:document.querySelectorAll('dialog[open]').length===0}:null })()`);
  ...
  await delay(500);
  const batchResultText = await evaluate(`document.querySelector('.speaker-review-durable-result')?.innerText||document.querySelector('[data-speaker-review-batch-complete]')?.innerText||window.storyAudioAppState.bookVoiceRegistry.speakerSuggestions?.lastResult||""`);
