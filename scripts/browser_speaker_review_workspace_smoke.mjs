...
try {
  ...
  const voiceEditingDeferred = await evaluate(`(() => { const card=document.querySelector(${JSON.stringify(attr("data-speaker-suggestion-card", key2))}); return !card?.querySelector('[data-speaker-focus-target="voice-mode"]') && card?.querySelector('[data-speaker-suggestion-voice-mode]')?.disabled===true && card?.querySelector('[data-speaker-suggestion-voice]')?.disabled===true && card?.querySelector('[data-speaker-suggestion-voice-scope]')?.disabled===true && card?.innerText.includes("Bước này chỉ chốt người nói/Character") })()`);
  ...
  const correctionHistoryVisible = await evaluate(`document.querySelector(${JSON.stringify(attr("data-speaker-suggestion-card", key1))})?.innerText.includes("Audit #") && document.querySelector(${JSON.stringify(attr("data-speaker-suggestion-card", key1))})?.innerText.includes("Bước này chỉ chốt người nói/Character")`);
