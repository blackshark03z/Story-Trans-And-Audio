...
try {
  ...
  await evaluate(`(() => { for(let timerId=1;timerId<10000;timerId+=1)clearInterval(timerId); return true; })()`);
  ...
  const journeyInfrastructure = await evaluate(`(() => { state.productionPrepare.readiness={runtime_mode:"PRODUCTION",prepare_allowed:false,start_render_allowed:false,operator_authentication_verified:false,blockers:[{code:"AUTH_BOOTSTRAP_MISSING",message:"Launcher chưa cấp phiên vận hành cho ứng dụng."}]}; renderProductionShell(); const visible=element=>getComputedStyle(element).display!=="none"&&element.getBoundingClientRect().width>0,primary=[...document.querySelectorAll("#productionTaskWorkspace .primary")].filter(visible).map(element=>element.textContent.trim()),back=document.querySelector("#productionBackToVoices"); return {state:document.querySelector("#productionStateCard").dataset.productionState,primary,back:visible(back)?back.textContent.trim():null,summary:document.querySelector("#productionStateExplanation").textContent}; })()`);
  const inspectBackLayout = async (width, height) => {
    await send("Emulation.setDeviceMetricsOverride", { width, height, deviceScaleFactor: 1, mobile: false });
    return evaluate(`(()=>{const primary=document.querySelector("#productionPrimaryAction").getBoundingClientRect(),back=document.querySelector("#productionBackToVoices").getBoundingClientRect();return{primary:{top:primary.top,left:primary.left,width:primary.width,height:primary.height},back:{top:back.top,left:back.left,width:back.width,height:back.height},horizontal:document.documentElement.scrollWidth>innerWidth+1}})()`);
  };
  const voiceReturnLayout1366 = await inspectBackLayout(1366, 768);
  const voiceReturnLayout520 = await inspectBackLayout(520, 900);
  await send("Emulation.setDeviceMetricsOverride", { width: 1366, height: 768, deviceScaleFactor: 1, mobile: false });
  const voiceReturn = await evaluate(`(async()=>{const savedLoad=loadBookVoiceRegistry,savedApi=api,calls=[];loadBookVoiceRegistry=async()=>({});api=async(path,options={})=>{calls.push({path:String(path),method:String(options.method||"GET").toUpperCase()});return{}};state.productionRange={...state.productionRange,bookId:91,fromChapter:1,toChapter:5,skipCompleted:true};rememberProductionWorkingContext({bookId:91,fromChapter:1,toChapter:5,skipCompleted:true});renderProductionShell();document.querySelector("#productionBackToVoices").click();await new Promise(resolve=>setTimeout(resolve,20));const result={route:state.currentRoute,hash:window.location.hash,voicesOpen:state.bookVoiceRegistry.workflowSections?.voices===true,mutations:calls.filter(call=>call.method!=="GET")};loadBookVoiceRegistry=savedLoad;api=savedApi;setAppRoute("production");return result})()`);
  ...
  const preparedEditCancel = await evaluate(`(async()=>{const savedApi=api,savedLoad=loadProductionTaskProjection,savedOpen=openPreRenderConfigurationTarget,savedConfirm=window.confirm,calls=[],targets=[],prompts=[];state.productionPreflight={...(state.productionPreflight||{}),execution_readiness:{prepared_job:{job_id:9001,status:"prepared"}}};renderProductionShell();const button=document.querySelector("#ownerCancelPreparedEdit"),label=button?.textContent?.trim()||"";window.confirm=message=>{prompts.push(String(message));return false};button?.click();await new Promise(resolve=>setTimeout(resolve,30));const declineCalls=calls.length;window.confirm=message=>{prompts.push(String(message));return true};api=async(path,options={})=>{calls.push({path:String(path),method:String(options.method||"GET")});return {status:"cancelled"}};loadProductionTaskProjection=async()=>state.productionProjection;openPreRenderConfigurationTarget=async target=>{targets.push(String(target))};button?.click();await new Promise(resolve=>setTimeout(resolve,50));const result={label,declineCalls,calls:[...calls],target:targets[0]||"",prompt:prompts.at(-1)||"",confirmAttempts:prompts.length};api=savedApi;loadProductionTaskProjection=savedLoad;openPreRenderConfigurationTarget=savedOpen;window.confirm=savedConfirm;return result})()`);
  ...
  const monitorJobsNavigation = await evaluate(`(async()=>{const button=document.querySelector('[data-render-open-jobs]'),label=button?.textContent?.trim()||'';button?.click();await new Promise(resolve=>setTimeout(resolve,30));const route=state.currentRoute;setAppRoute('production');renderProductionShell();return{label,route}})()`);
  const jobsRecoveryActions = await evaluate(`(async()=>{const savedAct=window.act,savedConfirm=window.confirm,calls=[],prompts=[];window.act=async(id,action)=>{calls.push({id:Number(id),action});return true};window.confirm=message=>{prompts.push(String(message));return true};state.jobs=[{id:9001,book_id:91,book_title:'Sách kiểm thử cô lập',from_chapter:401,to_chapter:401,status:'running',total_chapters:1,actual_completed:0,completed_segments:3,failed_segments:0,pending_segments:5,actions:{can_pause:true,can_cancel:true,can_resume:false,can_retry:false,can_start:false,can_open_audio:false}}];setAppRoute('jobs');renderJobsPage();const labels=[...document.querySelectorAll('#jobsPageList .job-actions button')].map(button=>button.textContent.trim());document.querySelector('[data-job-action="pause"]')?.click();document.querySelector('[data-job-action="cancel"]')?.click();await new Promise(resolve=>setTimeout(resolve,50));const result={labels,calls:[...calls],prompt:prompts[0]||'',technicalStart:document.querySelector('#jobsPageList')?.innerText.includes('START_RENDER')||false};window.act=savedAct;window.confirm=savedConfirm;setAppRoute('production');renderProductionShell();return result})()`);
  const jobsRecoveryVariants = await evaluate(`(async()=>{const savedAct=window.act,calls=[];window.act=async(id,action)=>{calls.push({id:Number(id),action});return true};setAppRoute('jobs');state.jobs=[{id:9002,book_id:91,book_title:'Sách kiểm thử cô lập',from_chapter:401,to_chapter:401,status:'paused',total_chapters:1,actions:{can_pause:false,can_cancel:true,can_resume:true,can_retry:false,can_start:false,can_open_audio:false}}];renderJobsPage();const pausedLabels=[...document.querySelectorAll('#jobsPageList .job-actions button')].map(button=>button.textContent.trim());document.querySelector('[data-job-action="resume"]')?.click();state.jobs=[{id:9003,book_id:91,book_title:'Sách kiểm thử cô lập',from_chapter:401,to_chapter:401,status:'failed',total_chapters:1,actions:{can_pause:false,can_cancel:false,can_resume:false,can_retry:true,can_start:false,can_open_audio:false},is_historical_output:false}];renderJobsPage();const failedLabels=[...document.querySelectorAll('#jobsPageList .job-actions button')].map(button=>button.textContent.trim());document.querySelector('[data-job-action="retry"]')?.click();await new Promise(resolve=>setTimeout(resolve,40));const result={pausedLabels,failedLabels,calls:[...calls]};window.act=savedAct;setAppRoute('production');renderProductionShell();return result})()`);
  ...
  const qaHandoff = await evaluate(`(async()=>{const savedOpen=openAudioReviewFromProduction,calls=[];openAudioReviewFromProduction=async vm=>{calls.push({task:vm?.task_type,artifact:vm?.qa?.artifact_id||null})};const player=document.querySelector('#productionQaAudio'),note=document.querySelector('#productionQaNote'),actions=document.querySelector('#productionQaActions'),button=document.querySelector('#productionPrimaryAction');button?.click();await new Promise(resolve=>setTimeout(resolve,30));const result={calls,player:!!player,note:!!note,qaActionsHidden:!!actions?.classList.contains('hidden'),handoff:document.querySelector('#productionTaskContent')?.innerText||'',label:button?.textContent?.trim()||''};openAudioReviewFromProduction=savedOpen;return result})()`);
  ...
  const rangeCommandContinuity = await evaluate(`(async()=>{const savedRange=state.productionRange,savedProjection=state.productionProjection,savedLoad=loadProductionTaskProjection,savedSync=syncCanonicalProductionContext,savedRender=renderProductionShell,savedPreflight=state.productionPreflight,base=JSON.parse(JSON.stringify(state.productionProjection));state.productionRange={bookId:91,fromChapter:500,toChapter:509,skipCompleted:true,readiness:{scope:{book_id:91,from_chapter:500,to_chapter:509},chapters:[]}};base.range_identity='book:91:500-509';const chapterProjection=JSON.parse(JSON.stringify(base));chapterProjection.range_identity='book:91:503-503';chapterProjection.canonical_task.task_key='chapter:9203:HUMAN_QA:artifact:999';const refreshed=JSON.parse(JSON.stringify(base));refreshed.canonical_task.task_key='range:91:500-509:COMPLETE';let reloads=0;const rendered=[];loadProductionTaskProjection=async()=>{reloads+=1;state.productionProjection=refreshed;state.productionProjectionKey=refreshed.canonical_task.task_key;renderProductionShell();return refreshed};syncCanonicalProductionContext=async()=>{};renderProductionShell=(...args)=>{rendered.push(state.productionProjection?.range_identity||null);return savedRender(...args)};state.productionProjection=base;state.productionProjectionKey=base.canonical_task.task_key;const envelope={schema:'story-audio-production-command/v1',command_id:'pc-range-continuity',command_type:'HUMAN_QA_ACCEPT',idempotency_key:'range-continuity-key',scope:{artifact:{id:999}},outcome:'APPLIED',submitted_count:1,applied_count:1,failed_count:0,applied_items:[{chapter_number:503}],failed_items:[],operator_message:'Đã ghi QA.',resulting_task_projection:chapterProjection,resulting_preflight:null,asynchronous_reference:null,state_tokens:{task_projection:'token',preflight:null}};await applyProductionCommandEnvelope(envelope,state.productionInteractionEpoch);const result={reloads,identity:state.productionProjection?.range_identity,range:{book:state.productionRange?.bookId,from:state.productionRange?.fromChapter,to:state.productionRange?.toChapter},renderedMismatch:rendered.includes('book:91:503-503')};state.productionRange=savedRange;state.productionProjection=savedProjection;state.productionPreflight=savedPreflight;loadProductionTaskProjection=savedLoad;syncCanonicalProductionContext=savedSync;renderProductionShell=savedRender;renderProductionShell();return result})()`);
  ...
  const prepareSkipCompleted = await evaluate(`(async()=>{
    const savedApi=api,savedCommand=runProductionCommand,savedLoadJobs=loadJobs,savedRange=state.productionRange,savedReadiness=state.productionPrepare.readiness,savedResult=state.productionPrepare.result;
    const calls=[];
    try{
      state.productionRange={bookId:1,fromChapter:6,toChapter:8,skipCompleted:true};
      state.productionPrepare.readiness={runtime_mode:'PRODUCTION',prepare_allowed:true};
      api=async url=>{
        if(url.startsWith('/api/production/batch-plan?'))return{summary:{total:3,eligible:2,already_complete:1,excluded:1,blocked:0},plan_fingerprint:'a'.repeat(64)};
        if(url==='/api/production/prepare-readiness')return{runtime_mode:'PRODUCTION',prepare_allowed:true};
        throw new Error('Unexpected regression API '+url);
      };
      runProductionCommand=async args=>{calls.push(args);return{outcome:'APPLIED'}};
      loadJobs=async()=>{};
  ...
  if (journeyG.primary.length !== 1 || journeyG.primary[0] !== "Mở Duyệt audio" || !journeyG.body.includes("Audio đã được bàn giao sang Duyệt audio") || journeyG.body.includes("Ghi chú QA")) throw new Error(`Production did not hand off QA cleanly: ${JSON.stringify(journeyG)}`);
  if (qaHandoff.player || qaHandoff.note || !qaHandoff.qaActionsHidden || qaHandoff.calls.length !== 1 || qaHandoff.calls[0].task !== "HUMAN_QA" || qaHandoff.label !== "Mở Duyệt audio") throw new Error(`Production QA handoff is not single-home: ${JSON.stringify(qaHandoff)}`);
  if (journeyInfrastructure.state !== "INFRASTRUCTURE_BLOCKED" || journeyInfrastructure.primary.length !== 1 || journeyInfrastructure.primary[0] !== "Kiểm tra lại môi trường" || journeyInfrastructure.back !== "Quay lại cấu hình giọng" || !journeyInfrastructure.summary.includes("chưa sẵn sàng")) throw new Error(`Infrastructure readiness was not safe: ${JSON.stringify(journeyInfrastructure)}`);
  if (voiceReturnLayout1366.horizontal || voiceReturnLayout1366.primary.left <= voiceReturnLayout1366.back.left || !voiceReturnLayout1366.back.width || voiceReturnLayout520.horizontal || voiceReturnLayout520.primary.top >= voiceReturnLayout520.back.top || voiceReturnLayout520.primary.width !== voiceReturnLayout520.back.width) throw new Error(`Voice return action layout failed: ${JSON.stringify({voiceReturnLayout1366,voiceReturnLayout520})}`);
  if (voiceReturn.route !== "assignment" || !voiceReturn.hash.includes("book=91&from=1&to=5") || !voiceReturn.hash.includes("assignment_focus=voices") || !voiceReturn.hash.includes("skip_completed=1") || !voiceReturn.voicesOpen || voiceReturn.mutations.length) throw new Error(`Voice configuration return did not preserve scope safely: ${JSON.stringify(voiceReturn)}`);
